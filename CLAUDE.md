...
- **DCPS framing and geometry are native metadata.** A vector-width trial must
  ...
  Fit outer plot limits to eligible current while retaining internal gaps and
  cell identities; do not drop archived rows or change QC/coverage to crop a plot.
...
- **The `cur_manual` flag position must never claim a review that did not
  ...
  called the panel.
- **A detected Data type is a FACT; ambiguity is the only editable state.**
  `detect_seaguard_data_type` (`QCS_DataHandler.py`) times a scalar session and
  identifies Mooring or Profile; a conclusive result locks the combobox, just
  as a DCPS binary layout locks it to Doppler.  Fewer than two usable timestamps,
  a read failure, or the archive's proven short+slow conflict remains explicitly
  ambiguous and editable.  If the scalar rule is ever retuned, recalibrate it
  against the archive's own FUNDEIO / PERFIL folders - that is the only ground
  truth there is.
...
- **Synchronous Qt loading uses `QCS_QtTheme.wait_cursor`.** Start it before
  workbook inspection, on the initiating window. Qt's stock Windows Wait/Busy
  resources can be static hourglasses; the helper loads the native animated
  cursor and limits it to that window. Keep dialogs interactive and restore
  cursor/filter state on nested calls and exceptions.
